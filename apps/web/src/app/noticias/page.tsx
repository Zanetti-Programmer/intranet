...
import { Plus, Loader2, Search, Newspaper, X, ChevronDown, ChevronUp, Image, Trash2, Eye } from "lucide-react";
...
function ArticleCard({ article, canManage, onDelete, onPublish, index }: {
  article: Article; canManage: boolean; onDelete: (id: string) => void;
  onPublish: (id: string) => void; index: number;
}) {
...
function NewArticleModal({ type, onSubmit, onClose }: {
  type: "news" | "blog"; onSubmit: (d: any) => Promise<void>; onClose: () => void;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">{type === "news" ? "Nova notícia" : "Novo post no blog"}</h3>
        ...
        <div><Label className="text-xs">Conteúdo *</Label>
          <Textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="Escreva o conteúdo completo..." className="mt-1 resize-none min-h-[160px]" rows={7} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Tags (vírgula)</Label><Input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="ex: rh, evento" className="mt-1 h-9" /></div>
        ...
        <div>
          ...
          <div onClick={() => fileRef.current?.click()}
            ...
            <Image style={{ width: 16, height: 16 }} className="text-muted-foreground" />
...
export default function NoticiasPage() {
  ...
  const { articles, loading, createArticle, deleteArticle, publishArticle, canSeeDrafts } = useArticles("news");
    ...
    <DashboardLayout pathname={pathname}>
      <div className="max-w-4xl mx-auto px-5 py-6 space-y-5">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-semibold flex items-center gap-2"><Newspaper className="w-5 h-5 text-primary" />Notícias</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Comunicados oficiais e novidades da empresa</p>
          </div>
          {canCreate && <Button onClick={() => setShowModal(true)} size="sm" className="gap-1.5 shrink-0"><Plus className="w-4 h-4" />Nova notícia</Button>}
        </div>

        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar notícias..." className="pl-9 h-9" />
        </div>

        {canSeeDrafts && (
          <div className="flex gap-1.5">
            {[["published","Publicadas"],["draft","Rascunhos"],["all","Todas"]] .map(([v, l]) => (
        ...
        {loading ? <div className="flex justify-center py-12"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
          : filtered.length === 0 ? (
            <div className="text-center py-16 space-y-2">
              ...
              {canCreate && <p className="text-sm text-muted-foreground">Clique em "Nova notícia" para publicar.</p>}
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map((a, i) => (
                <ArticleCard key={a.id} article={a} canManage={canSeeDrafts} index={i}
      ...
      <AnimatePresence>
        {showModal && <NewArticleModal type="news" onSubmit={createArticle} onClose={() => setShowModal(false)} />}
      </AnimatePresence>
    </DashboardLayout>
  );
}
